...
//                        }
    ...
    public function getCustomerIndex() {
        ...
        $pagedata['mainHeading'] = "Ticket";
        $this->load->template('ticket/customercustomer/index', $pagedata);
    }
...
//    }
    ...
    public function getEmployeeGridView($org_id) {
	 ...
	 public function printview($ticket_id) {
        ...
        $ticketAssignData = $this->crm->getData('ticket_assign','ticket_assign.ticket_assign_id,user.user_mobile,user.user_code,user.address1,user.address2,user.user_city,user.user_postcode,ticket_assign.assigned_by,ticket_assign.assigned_by,ticket_assign.ticket_assign_at,user.user_name as assignee,uasb.user_name as assigned_by_user,working_hour.minutes,ticket_assign.user_id as assigni_id', $where1, $join1, 'ticket_assign_at', 'DESC', '', '', '');
        $time_total = 0;
        ...
        if (!empty($ticketData)) {
          echo  $this->load->view('ticket/employee/print_page',array('ticket_detail' => $ticketData, 'ticket_assign_data' => $emplyeeTime, 'history' => $ticket_history, 'total_time' => $time_total),true);
            die;
        } else {
            echo '0';
            die;
...
//                $link
    ...
    public function getToDoData($org_id, $user_id = FALSE) {
        ...
        if ($user_id) {
            $where = "(ticket_assign.user_id = $user_id) AND (current_working_user = 1) AND ((ticket_status = 'Open') OR (ticket_status = 'Pending'))";
//            $where = array('organisation_id' => $org_id, 'ticket_status' => 'Open', 'ticket_assign.user_id' => $user_id, 'current_working_user' => 1);
        } else {
            $where = "((ticket_status = 'Open') OR (ticket_status = 'Pending'))";
        }
    ...
    public function getDoingData($org_id, $user_id = FALSE) {
        ...
        if ($user_id) {
            $where = array('ticket_status' => 'Doing', 'ticket_assign.user_id' => $user_id, 'current_working_user' => 1);
        } else {
            $where = array('ticket_status' => 'Doing');
        }
    ...
    public function getDoneData($org_id, $user_id = FALSE) {
        ...
        if ($user_id) {
            $where = array('ticket_status' => 'Solved', 'ticket_assign.user_id' => $user_id, 'current_working_user' => 1);
        } else {
            $where = array('ticket_status' => 'Solved');
        }
        $backlogData = $this->crm->getData('ticket', 'ticket.*,ticket_assign.assigned_by,ticket_assign.assigned_by,ticket_assign_at', $where, $join, FALSE, FALSE, FALSE, FALSE, FALSE, array('ticket_id'));
...
//                    $where = "(`ticket_assign`.`current_working_user` = 1) AND (`ticket`.`ticket_status` = 'Open' OR `ticket`.`ticket_status` = 'Pending')";
		   $where =   "(`ticket`.`ticket_status` != 'Closed')";
                } else {
                  $join=array();  
...
//               echo $this->db->last_query();
            ...
            case '3': $heading = 'All unsolved tickets';
                ...
                if ($user_id == '') {
                    $where = "((ticket.ticket_status = 'Open') OR (ticket.ticket_status = 'Doing') OR (ticket.ticket_status = 'Pending'))";
                } else {
                  
                    $where = "(ticket_assign.user_id = $user_id) AND ((ticket.ticket_status = 'Open') OR (ticket.ticket_status = 'Doing') OR (ticket.ticket_status = 'Pending'))";
...
//		echo $this->db->last_query();die;
            ...
            case '7': $heading = 'Recently Closed';
                ...
                if ($user_id == '') {
                    $where = "(ticket_history_status = 'Closed') AND (ticket.ticket_status = 'Closed') AND (ticket_updated BETWEEN '$last_time_stamp'  AND '$time_stamp')";
                } else {
...
//		 echo $user_id.'dsdsd';
        ...
        if ($user_id) {
            ...
            $unsolvedwhere = array('ticket.ticket_status != ' => 'Solved', 'ticket.ticket_status !=' => 'Pending');
